...
- one compact rules/search core shared by every mode;
...
- separate handcrafted evaluators for Total War and Infiltration;
...
- **Engine terminal rules, inherited by every mode, with the result each mode
  declares (`ModeRules::stalemate_loses`, `ModeRules::repetition_draws`):**
  - a player with no legal move ends the game — a **draw** in Total War and
    Infiltration, a **loss for the stuck side** in Intransitive;
  - the third occurrence of a position on the game path is a draw in Total War
    and Infiltration, and **nothing at all** in Intransitive.
...
1. **Infiltration wipeouts are draws.** Infiltration has no annihilation win
   ...
   is a stalemate draw rather than an undefined state. This resolves the
   previously open question rather than adding a mode-specific rule. The same
   position in Intransitive is a loss, from the same absent annihilation rule
   plus `stalemate_loses` — again a rule fact rather than a mode branch.
2. **Stalemate being a draw is what licenses the uncapturable-piece proof.**
   ...
   guarantee at least a draw, and the bound in section 8 would be unsound. So
   `immortality_prevents_loss` and `territory_majority_prevents_loss` both
   require `!stalemate_loses`, and neither fires in Intransitive.
3. **Stalemate being a loss is zugzwang, which unlicenses the whole selective
   layer.** The null move, both futility rules, move-count pruning, the
   late-move reductions and the quiescence cutoff all assume having to move is
   never worse than passing. `Searcher::begin_search` therefore turns the layer
   off in a mode that declares `stalemate_loses`, rather than patching it rule
   by rule.

The `NoMoveOutcome` search setting is gone: the rule belongs to `ModeRules`,
and a setting that could contradict the mode was a way to score one mode's
position under another's rules.
...
| --- | --- | --- | --- | --- | --- |
| Total War | yes | yes (territory) | yes | no | with a tile majority |
...
├── Cargo.toml
├── README.md
├── docs/           PLAN.md (this file), reference docs, experiment logs
└── src/
...
Opening books have moved off this list and are implemented; see section 8.1.
They are not a search feature and never enter one: a book is built offline,
read offline, and changes no node the engine visits. That is why they did not
need the significance testing the items above do.
...
piece is worth 70 in Total War and 35 in Infiltration, and one fixed margin
would prune almost nothing in the first mode and almost everything in the
second.

### 8.1 Opening books (implemented)

A book is a canonicalized position graph on disk, built by best-first drop-out
expansion and read back by `book show`. It exists because depth and breadth are
different questions and one search cannot answer both: node counts roughly
double per ply, so a single depth-50 search of the Infiltration start is about
3e16 nodes, while a fifty-ply *graph* whose every node carries its own
depth-13-to-22 search is an overnight run. The second is also the better
artifact, because every ply gets a fresh horizon rather than inheriting one.

Design commitments, each of which is a rule the implementation may not break:

1. **Canonical keys.** Positions are stored under the representative of their
   symmetry class, so lines reaching one board by different move orders share
   one analysis. The transform group is exact and tested
   (`the_symmetry_group_preserves_play_and_evaluation`,
   `perft_agrees_across_the_symmetry_group`). Reversing files is the fold that
   pays; the colour swap is sound but nearly inert, because it flips the side
   to move and a game with a fixed first player never reaches the image.
2. **Node budgets, tapered by ply, never wall-clock.** Same rule as the arena,
   same reason: a book built on one machine must be the book another machine
   would have built. A single-threaded run is byte-reproducible.
3. **Append-only storage.** An interrupted run keeps what it finished, and a
   larger budget appends rather than rewrites. Records are superseded by depth,
   so a later shallow pass can never undo a deep one.
4. **No mixing.** `meta.txt` fingerprints the weights, rules version, mode, and
   symmetry, and a book refuses anything produced under different ones. Scores
   from different weights are not comparable, and a book that averaged them
   would present a difference between conventions as a difference between
   moves.
5. **Cycles are cut, not assumed away.** The position graph is not a tree;
   pieces walk back. Backing up scores a repetition on the current path as a
   draw and only memoizes a value when no cycle was cut beneath it. This is the
   pragmatic answer to graph history interaction, and the honest description of
   it is that it never invents a value but can score a position differently
   from two lines when its only refutation is a repetition.

Website export is implemented by `book export`. `nodes.log` and `paths.log`
remain the authoritative artifact; the command walks each stored edge back
through its canonical transform and emits a bounded JSON response tree in real
board coordinates. Cycles are marked and cut, ranked alternatives retain their
backed-up scores, and the principal variation becomes the website's core main
line. Human names deliberately remain website data so rebuilding analysis
cannot erase them.
...
engine — profiles live in `frontend/src/engine/bots/profiles.ts` and the root sampler
in `frontend/src/engine/bots/engine.ts`, over the ordinary MultiPV WASM entry point.
...
- `arena.rs` cannot measure the ladder, because the policy it would be
  measuring is not in this crate. `frontend/scripts/botArena.mts` fills that
...
- Stalemate ends the game in Go and Rust, with the result the mode declares;
  `RULES_VERSION` is 3.
...
- Official no-move, repetition, and Infiltration elimination rules decided and
  implemented in both languages, and since made per-mode facts rather than
  engine-wide ones.
...
5. Investigate the self-play truncation rate: a substantial share of games
   reach the 400-ply ceiling, which suggests a no-progress rule is worth
   specifying per mode. Intransitive makes this pressing rather than tidy — it
   has no repetition draw, so the ply ceiling is the *only* thing that ends a
   shuffle there, and every truncated game is scored as a draw it did not earn.
6. Rebuild every opening book and re-measure Intransitive's fitted constants:
   both went stale at `RULES_VERSION` 3. See the stale-artifact note in
   `README.md`.
7. Measure what turning the selective layer off costs Intransitive, at a fixed
   node budget, so the trade is a number rather than an assertion.
